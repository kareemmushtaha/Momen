...
class ContactUsDataTable extends DataTable {
	 ...
	 * Auto Ajax Method By Baboon Script [It V 1.0 | https://ecovne.com]
	...
	public function dataTable(DataTables $dataTables, $query) {
		return datatables($query)
			->addColumn('actions', 'admin.contactus.buttons.actions')
			->addColumn('status_msg', 'admin.contactus.buttons.status_msg')
			->addColumn('replay_at', 'admin.contactus.buttons.replay_at')
			->addColumn('move_to', 'admin.contactus.buttons.move_to')
			...
			->rawColumns(['checkbox', 'show_action', 'actions', 'replay_at', 'move_to', 'user', 'date']);
	}
	...
	public function query() {
		return ContactUs::query()->orderBy('id', 'desc')->where(function ($q) {
				if (request('status_msg')) {
					return $q->where('status_msg', request('status_msg'));
				}
			})->where(function ($q) {
				if (request('folder')) {
					return $q->where('move_to', request('folder'));
				}
			})->select('contact_us.*');
